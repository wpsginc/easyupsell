...
- LOW = Nice-to-have or niche (specialty tools, luxury upgrades)"""
        ...
        try:
            ...
            for j, item in enumerate(batch):
                llm_match = next((r for r in llm_items if r.get("index") == j + 1), None)
                if llm_match:
                    ...
                    item["priority"] = llm_match.get("priority", "Medium")
                    item["sourcing_notes"] = llm_match.get("sourcing_notes", "")
                enriched.append(item)

        except Exception as e:
            logger.warning(f"LLM enrichment failed for batch {i//batch_size + 1}: {e}")
...
def write_opportunities_excel(
    ...
    detail_fill = PatternFill(start_color="E8F0FE", end_color="E8F0FE", fill_type="solid")
    ...
    sorted_opps = sorted(
        opportunities,
        key=lambda x: (priority_order.get(x.get("priority", ""), 4), -x.get("category_count", 0)),
    )

    # Data
    for row_idx, opp in enumerate(sorted_opps, 2):
        priority = opp.get("priority", "")
        fill = high_fill if priority == "High" else medium_fill if priority == "Medium" else low_fill
        ...
        for col_idx, (_, field) in enumerate(opp_fields, 1):
            ...
            if field in ("price_range_low", "price_range_high") and isinstance(value, (int, float)):
    ...
    # ── Sheet 2: Gap Detail ──
    ws_detail = wb.create_sheet("Gap Detail")
