...
def get_batch_llm_validation(
    ...
    for i, item in enumerate(items_to_evaluate):
        ...
        if item.get("same_category"):
            details.append(
                "⚠️ SAME CATEGORY as source — valid ONLY if accessory/part/upgrade"
            )
...
IMPORTANT: Return ONLY the JSON array, no other text."""
        ...
        return [
            {
                "valid": None,
                "confidence": 0,
                "reason": f"Provider {provider} not supported for batch",
            }
    ...
    if not isinstance(result, list):
        ...
        return [
            {"valid": None, "confidence": 0, "reason": "Batch parse failed"}
            for _ in items_to_evaluate
        ]
...
def _call_azure_batch(
    prompt: str, expected_count: int, deployment: Optional[str] = None
) -> list:
    ...
    try:
        ...
        if not parsed:
            print(
                f"  ⚠️ Azure batch: got response but parsed 0 items. Raw: {content[:300]}"
            )
        return parsed
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["azure", "openai", "ollama", "litellm", "athena"],
    )
    parser.add_argument(
        "--categories",
        default=str(settings.DATA_DIR / "categories.json"),
        help="Categories file",
    )
    parser.add_argument(
        "--output",
        default=str(settings.DATA_DIR / "validated_pairings"),
        help="Output path",
    )
    ...
    if valid:
        ...
        for r in sorted(valid, key=lambda x: -x.get("suggested_weight", 0))[:10]:
            ...
            print(
                f"    Weight: {r.get('suggested_weight')}, Type: {r.get('relationship_type')}"
            )
