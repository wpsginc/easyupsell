...
def _migrate_csv(db: RecommendationDB, csv_path: Path) -> dict[str, int]:
    counts = {"total": 0, "active": 0, "pending_review": 0, "rejected": 0, "errors": 0}
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            payload = {
                ...
                "target_netsuite_id": (row.get("recommended_netsuite_id") or "").strip()
                or None,
...
def _migrate_xlsx(db: RecommendationDB, xlsx_path: Path) -> dict[str, int]:
    ...
    try:
        ...
        for values in ws.iter_rows(min_row=2, values_only=True):
            ...
            target_sku = str(row.get("target_sku") or "").strip()
            ...
            try:
                ...
                raw_score = float(str(val) if val is not None else 0)
            except (ValueError, TypeError):
                raw_score = 0.0
            score = _clamp_score(raw_score / 5)
            ...
            try:
                ...
                print(f"Skipping XLSX row due to error: {e}", file=sys.stderr)
                counts["errors"] += 1
    finally:
        wb.close()
...
def migrate_csv_to_db(
    csv_path: Path, xlsx_path: Path | None, db_path: Path
) -> dict[str, int]:
    db = RecommendationDB(db_path)
    db.init_db()

    csv_counts = _migrate_csv(db, csv_path)
    xlsx_counts = {"total": 0, "active": 0, "pending_review": 0, "rejected": 0, "errors": 0}
    if xlsx_path:
    ...
    return {
        ...
        "errors": csv_counts["errors"] + xlsx_counts["errors"],
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Migrate recommendation CSV/XLSX data into SQLite DB"
    )
    parser.add_argument(
        "--csv", required=True, help="Path to full_recommendations_v2.csv"
    )
    parser.add_argument(
        "--xlsx", required=False, help="Optional path to dark_horse reviewed workbook"
    )
    ...
    print(
        "Migrated {total} recommendations ({active} active, {pending_review} pending_review, {rejected} rejected, {errors} errors)".format(
            **summary
        )
    )
    return 0
