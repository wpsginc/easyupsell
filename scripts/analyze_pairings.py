...
def get_sales_velocity(client: BigCommerceClient, months: int = 6) -> dict:
    ...
    try:
        orders = client.get_orders(min_date=min_date_str, status_id=BC_STATUS_COMPLETED)
        orders.extend(
            client.get_orders(min_date=min_date_str, status_id=BC_STATUS_SHIPPED)
        )
    ...
    for i, order in enumerate(orders[:200]):  # Sample 200 orders for velocity
        try:
            ...
            for p in products:
                ...
                if product_id and product_id not in product_categories:
                    try:
                        ...
                        print(
                            f"  ⚠️ Failed to fetch product {product_id} categories "
                            f"({type(e).__name__})"
                        )
            ...
            print(f"  ⚠️ Failed processing order {order_id} ({type(e).__name__})")
            continue
...
def get_category_margins(bc_client: BigCommerceClient) -> dict:
    ...
    FROM item i
    LEFT JOIN pricing p ON p.item = i.id AND p.pricelevel = {NETSUITE_PRICELEVEL_STANDARD}
    ...
    for prod in bc_products:
        sku = prod.get("sku", "")
        price = safe_float(prod.get("price"), default=0.0)

        # Find NS cost
        ns_item = ns_items.get(sku, {})
        cost = safe_float(
            ns_item.get("averagecost") or ns_item.get("cost"), default=0.0
        )
...
def generate_smart_pairings(categories: list[dict], limit: int = 50) -> list[tuple]:
    ...
    sorted_cats = sorted(
        categories, key=lambda c: c.get("product_count", 0), reverse=True
    )

    pairings = []
    names = [
        (c["name"], c.get("parent_id", 0)) for c in sorted_cats[:40]
    ]  # Top 40 by product count
...
def main():
    ...
    parser.add_argument(
        "--output", default="data/pairing_analysis.csv", help="Output CSV"
    )
    parser.add_argument("--provider", default="azure", help="LLM provider")
    parser.add_argument(
        "--skip-orders", action="store_true", help="Skip order analysis (faster)"
    )
    parser.add_argument(
        "--skip-margin", action="store_true", help="Skip NetSuite margin lookup"
    )
    ...
    if valid:
        ...
        for r in sorted(valid, key=lambda x: -x.get("opportunity_score", 0))[:5]:
            ...
            print(
                f"    Score: {r['opportunity_score']}, Type: {r['relationship_type']}"
            )
            print(
                f"    Source: {r['src_orders_6mo']} orders, Target: {r['tgt_orders_6mo']} orders"
            )
