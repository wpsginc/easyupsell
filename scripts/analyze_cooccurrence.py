...
def validate_since_date(since_date: str) -> str:
    """Validate SuiteQL date literal input (YYYY-MM-DD)."""
    try:
        parsed = datetime.strptime(since_date, "%Y-%m-%d")
    except ValueError as e:
        raise ValueError("since_date must be in YYYY-MM-DD format") from e
...
def get_orders_with_items(client: NetSuiteClient, since_date: str) -> list[dict]:
    ...
    since_date = validate_since_date(since_date)
...
def calculate_cooccurrence(
    orders: dict[str, set[str]], min_count: int = 3
) -> list[dict]:
    """
...
def save_results(results: list[dict], output_path: Path):
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["product_a", "product_b", "count", "support"]
        )
        writer.writeheader()
...
def main():
    ...
    parser.add_argument(
        "--months", type=int, default=12, help="Months of history to analyze"
    )
    parser.add_argument(
        "--min-count", type=int, default=3, help="Minimum co-occurrence count"
    )
    parser.add_argument(
        "--output",
        default="data/cooccurrence",
    ...
    since_date = (datetime.now() - timedelta(days=args.months * 30)).strftime(
        "%Y-%m-%d"
    )
