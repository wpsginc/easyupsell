...
from constants import BC_STATUS_COMPLETED, BC_STATUS_SHIPPED, DEFAULT_SAMPLE_SIZE
...
def safe_float(value, default: float = 0.0) -> float:
    """Best-effort float conversion for external/untrusted values."""
    if value is None:
        return default
...
def get_processed_categories(output_path: Path) -> set[str]:
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning:[/yellow] Could not read '{output_path.name}'. "
            f"Will reprocess all categories ({type(e).__name__})."
        )
...
def get_order_data(
    bc_client: BigCommerceClient, months: int = 6
    ...
    # Get orders
    try:
        orders = bc_client.get_orders(
            min_date=min_date_str, status_id=BC_STATUS_COMPLETED
        )
        orders.extend(
            bc_client.get_orders(min_date=min_date_str, status_id=BC_STATUS_SHIPPED)
        )
    ...
    sample_size = min(DEFAULT_SAMPLE_SIZE, len(orders))
    ...
    for i, order in enumerate(orders[:sample_size]):
        try:
            ...
            for p in products:
                ...
                price = safe_float(p.get("base_price"), default=0.0) * qty
                ...
                if product_id and product_id not in product_categories:
                    try:
                        ...
                        console.print(
                            f"[yellow]Warning:[/yellow] Failed to fetch product {product_id} "
                            f"categories ({type(e).__name__})."
                        )
            ...
            console.print(
                f"[yellow]Warning:[/yellow] Failed processing order {order_id} "
                f"({type(e).__name__})."
            )
            continue
...
def get_high_value_items_with_sales(
    ...
    products = bc_client.get_products(
    ...
    for p in products:
        price = safe_float(p.get("price"), default=0.0)
        product_id = p["id"]
        name = p.get("name", "")
    ...
    print(
        f"  Found {len(priority_items)} priority brand items, {len(regular_items)} regular"
    )
    print(
        f"  Selected: {priority_limit} priority + {regular_limit} regular = {len(candidates)}"
    )
    return candidates
...
def find_recommendations_for_category(
    ...
    for item in selected:
        ...
        if copurchase_count > 0:
            item_data["copurchase_text"] = (
                f"Co-purchased with '{category_name}': {copurchase_count} times"
            )
    ...
    try:
        ...
        batch_results = [
            {"valid": None, "reason": f"Batch error: {e}"} for _ in selected
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Full recommendation analysis with sales data"
    )
    parser.add_argument(
        "--top-categories",
        type=int,
        default=20,
        help="Number of top categories by sales",
    )
    parser.add_argument(
        "--items-per-category", type=int, default=6, help="Candidate items per category"
    )
    parser.add_argument(
        "--output",
        default=str(settings.DATA_DIR / "full_historical_recommendations.xlsx"),
        help="Output Excel file",
    )
    parser.add_argument(
        "--provider", default=settings.DEFAULT_PROVIDER, help="LLM provider"
    )
    parser.add_argument(
        "--skip-orders",
        action="store_true",
        help="Skip order analysis (use random categories)",
    )
    parser.add_argument(
        "--use-bq",
        action="store_true",
        help="Use BigQuery for category ranking (FAST, recommended)",
    )
    ...
    if args.use_bq:
        ...
        try:
            ...
            category_sales, item_sales, product_categories = get_order_data(
                client, months=6
            )
    ...
    top_categories = get_top_categories_by_sales(
        client, category_sales, limit=args.top_categories, use_cache=use_cache
    )
    print(f"\nTop {len(top_categories)} categories by sales:")
    for i, cat in enumerate(top_categories[:10]):
        print(
            f"  {i + 1}. {cat['name']} ({cat['order_count']} orders, ${cat['revenue']})"
        )
    ...
    high_value_items = get_high_value_items_with_sales(
        client, item_sales, product_categories, limit=300, use_cache=use_cache
    )
    ...
    if processed_categories:
        console.print(
            f"[yellow]Found {len(processed_categories)} already processed categories. Resuming...[/yellow]"
        )
    ...
    with Progress(
        ...
        task_id = progress.add_task(
            "Processing Categories...", total=len(top_categories)
        )
        ...
        for i, cat in enumerate(top_categories):
            ...
            if cat_name in processed_categories:
                progress.update(
                    task_id, advance=1, description=f"Skipping {cat_name} (Done)"
                )
                continue
            ...
            try:
                ...
                progress.console.print(
                    f"  ✓ {cat_name}: {valid_count}/{len(recs)} valid"
                )
...
def write_excel_output(csv_path: Path, excel_path: Path, fieldnames: list[str]):
    ...
    console.print(
        f"  ✅ {len(all_rows)} recommendations ({len(categories)} categories, {invalid_count} invalid) → single workbook"
    )
